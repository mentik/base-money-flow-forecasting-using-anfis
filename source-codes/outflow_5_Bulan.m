...
fprintf('******** Program Prediksi OutFlow ANFIS ******** \n');
...
model = [data_csv(:,1:2) ((data_csv(:,3)* 1.7) + (data_csv(:,4)* 1.8) + (data_csv(:,5)* 0.7) + (data_csv(:,6)* 3)) data_csv(:,7)];
file = model;
for i = 1:1:s 
    %Menjalankan simulasi
    t = s+1-i;
    split = length(file)-t; 
    %Mean Encoding Dummy Bulan (Training)
    R = f_mean_encoding(file(1:split,:),out);
    file_e = [file(:,1:3) f_encode(file(:,4),R)];
    %Normalisasi
    [pre_train ab] = f_normal(file_e(1:split,:)); %Normalisasi data pre_train
    pre_test = f_normal_x(file_e(split+1,:),ab); %Normalisasi pre_test ke ab
    ...
    fisx = genfis1(train,2,'gbellmf','constant');
    epoch_n = [30 0];
    dispOpt = zeros(1,4);
    %Menjalankan pelatihan
    fis = anfis(train,fisx,epoch_n,dispOpt);
    %Memprediksi test
...
fprintf('******** Simulasi Model Neuro Fuzzy Untuk Prediksi Outflow Telah Dijalankan ******** \n');