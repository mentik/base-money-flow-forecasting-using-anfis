...
file = [data_csv(:,1:2) ((data_csv(:,3)* 1.7) + (data_csv(:,4)* 1.8) + (data_csv(:,5)* 0.7) + (data_csv(:,6)* 3)) data_csv(:,7)];
for i = 1:1:s 
    ...
    dataset = f_swts_matrix(simulasi,p,in,out,[1 0]); % Variabel tambahan[HR Bulan]
    ...
    fis = anfis(train,fisx,epoch_n,dispOpt,[],1);
    %Memprediksi test
